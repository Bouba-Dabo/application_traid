    ...
    def load_rules(self, path: str):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                l = line.replace('AND', 'and').replace('OR', 'or')
                # Try to parse numeric score form: IF <expr> THEN +N # comment
                m_score = re.match(r'IF\s+(.+)\s+THEN\s+([+-]?\d+)\s*(?:#(.*))?$', l, re.IGNORECASE)
                if m_score:
                    expr = m_score.group(1)
                    score = int(m_score.group(2))
                    comment = m_score.group(3) or ''
                    self.rules.append(Rule(expr, score=score, action='', comment=comment))
                    continue

                # Fallback: IF <expr> THEN ACTION (BUY/SELL/HOLD)
                m = re.match(r'IF\s+(.+)\s+THEN\s+(\w+)(?:\s*#(.*))?', l, re.IGNORECASE)
                if m:
                    expr, action, comment = m.group(1), m.group(2).upper(), m.group(3) or ''
                    # Map legacy actions to scores
                    action_score_map = {'BUY': 3, 'ACHETER': 3, 'SELL': -3, 'VENDRE': -3, 'HOLD': 0}
                    score = action_score_map.get(action, 0)
                    self.rules.append(Rule(expr, score=score, action=action, comment=comment))
    ...
    def evaluate(self, indicators: Dict[str, Any], fundamentals: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for r in self.rules:
            ...
            if hit:
                ...
                triggered.append({'action': r.action, 'expr': r.expr, 'comment': r.comment, 'score': r.score})
